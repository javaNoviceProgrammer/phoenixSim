...
import photonics.ring.AddDropFifthOrder;

public class AddDropRingFifthOrderTabController extends AbstractTabController {
	...
	@FXML TextField kappa45TextField ;
    ...
    @FXML
    public void set45Kappa(){
        if(!kappa45TextField.getText().isEmpty()){
        	try {
                double kappa45 = evaluate(kappa45TextField.getText()) ;
                simDataBase.addNewVariable(new SimulationVariable("kappa45_()", "Kappa45 ()", new double[]{kappa45}));
                kappa45Label.setText("kappa_45 is set to " + kappa45 + "");
			} catch (Exception e) {
				kappa45TextField.setStyle("-fx-text-inner-color: red;");
				kappa45TextField.setOnKeyTyped(event->{
					kappa45TextField.setStyle("-fx-text-inner-color: black");
				});
			}
	...
	public void calculateSpectrum(){
		...
		double kappa45 = simDataBase.getVariable("kappa34_()").getValue(0) ;
		...
		if(dropLambda.isSelected()){
			...
			for(int i=0; i<lambda_nm.length; i++){
				...
				AddDropFifthOrder adr = new AddDropFifthOrder(phi_rad, loss, kappaIn, kappaOut, kappa12, kappa23, kappa34, kappa45) ;
				drop_dB[i] = Conversions.todB(adr.getS41().absSquared()) ;
			}
			simDataBase.addNewVariable(new SimulationVariable("drop_5thOrder_(dB)", "Drop (dB)", drop_dB));
			fig = createPlot(simDataBase.getVariable("lambda_(nm)"), simDataBase.getVariable("drop_5thOrder_(dB)")) ;
			showPlot(fig, plotPane);
		}
			...
			simDataBase.addNewVariable(new SimulationVariable("thru_5thOrder_(dB)", "Thru (dB)", thru_dB));
			fig = createPlot(simDataBase.getVariable("lambda_(nm)"), simDataBase.getVariable("thru_5thOrder_(dB)")) ;
			showPlot(fig, plotPane);
		}
	}
