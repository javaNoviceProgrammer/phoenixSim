...
import PhotonicElements.RingStructures.AddDrop.Graph.AddDropThirdOrder;
...
public class AddDropRingThirdOrderTabController extends AbstractTabController {
	...
	public void calculateSpectrum(){
		...
		if(dropLambda.isSelected()){
			...
			for(int i=0; i<lambda_nm.length; i++){
				...
				AddDropThirdOrder adr = new AddDropThirdOrder(phi_rad, loss, kappaIn, kappaOut, kappa12, kappa23) ;
				drop_dB[i] = Conversions.todB(adr.getS41().absSquared()) ;
			}
			simDataBase.addNewVariable(new SimulationVariable("drop_3rdOrder_(dB)", "Drop (dB)", drop_dB));
			fig = createPlot(simDataBase.getVariable("lambda_(nm)"), simDataBase.getVariable("drop_3rdOrder_(dB)")) ;
			showPlot(fig, plotPane);
		}
			...
			simDataBase.addNewVariable(new SimulationVariable("thru_3rdOrder_(dB)", "Thru (dB)", thru_dB));
			fig = createPlot(simDataBase.getVariable("lambda_(nm)"), simDataBase.getVariable("thru_3rdOrder_(dB)")) ;
			showPlot(fig, plotPane);
		}
	}
