...
public class AddDropRingBSTabController extends AbstractTabController {
    ...
    public void setR(){
        if(!rTextField.getText().isEmpty()){
        	try {
                double r = evaluate(rTextField.getText()) ;
                simDataBase.addNewVariable(new SimulationVariable("r_()", "Reflectivity ()", new double[]{r}));
                rLabel.setText("Reflectivity is set to " + r + "");
			} catch (Exception e) {
				rTextField.setStyle("-fx-text-inner-color: red");
				rTextField.setOnKeyTyped(event->{
					rTextField.setStyle("-fx-text-inner-color: black");
				});
			}
	...
	public void calculateSpectrum(){
		...
		double r = simDataBase.getVariable("r_()").getValue(0) ;
		...
		if(dropLambda.isSelected()){
			...
			for(int i=0; i<lambda_nm.length; i++){
				...
				double loss = pow(10, -loss_dB/10) ;
				AddDropSymmetricBS adr = new AddDropSymmetricBS(kappaIn, kappaOut, loss, phi_rad, new LumpedReflector(r, 0)) ;
				drop_dB[i] = Conversions.todB(adr.S41.absSquared()) ;
			}
			simDataBase.addNewVariable(new SimulationVariable("drop_1stOrder_BS_(dB)", "Drop (dB)", drop_dB));
			fig = createPlot(simDataBase.getVariable("lambda_(nm)"), simDataBase.getVariable("drop_1stOrder_BS_(dB)")) ;
			showPlot(fig, plotPane);
		}
			...
			for(int i=0; i<lambda_nm.length; i++){
				...
				double loss = pow(10, -loss_dB/10) ;
				AddDropSymmetricBS adr = new AddDropSymmetricBS(kappaIn, kappaOut, loss, phi_rad, new LumpedReflector(r, 0)) ;
				thru_dB[i] = Conversions.todB(adr.S21.absSquared()) ;
			}
			simDataBase.addNewVariable(new SimulationVariable("thru_1stOrder_BS_(dB)", "Thru (dB)", thru_dB));
			fig = createPlot(simDataBase.getVariable("lambda_(nm)"), simDataBase.getVariable("thru_1stOrder_BS_(dB)")) ;
			showPlot(fig, plotPane);
		}
